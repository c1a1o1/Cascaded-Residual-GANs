...
class create_model(Model):
    ...
    def WGAN_GP_loss_discriminator(self, real, fake, is_training, scope):
        with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
            alpha = tf.random_uniform(shape=[1, 1], minval=0.0, maxval=1.0)
            difference = fake - real
            interpolate = real + alpha * difference
            gradient = tf.gradients(self.build_discriminator(interpolate, is_training, scope, True), [interpolate])[0]
    ...
    def compute_loss(self, im_batch):
        ...
        with tf.name_scope("generator_loss"):
            ...
            # fake gan
            gen_loss_gan = (loss.wgan_loss_generator(im_batch[4]) + loss.wgan_loss_generator(im_batch[5])) / 2.0
            # generator loss
        ...
        with tf.name_scope("discriminator_loss"):
            discrim_loss_a = loss.wgan_loss_discriminator(im_batch[6], im_batch[4])
            discrim_loss_a += self.LAMBDA * self.WGAN_GP_loss_discriminator(im_batch[0], im_batch[2], self.is_training, 'Discrim1')
            discrim_loss_b = loss.wgan_loss_discriminator(im_batch[7], im_batch[5])
            discrim_loss_b += self.LAMBDA * self.WGAN_GP_loss_discriminator(im_batch[1], im_batch[3], self.is_training, 'Discrim2')
            ...
            with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
                ...
                for epoch in range(0, self._max_step):
                    ...
                    for i in range(0, self._save_every_iterations // 4):
                        ...
                        image_pol = []
                        for j in range(4):
                            image_opt.append(np.float32(scipy.misc.imread("./data/0.51m/train/opt/%.8d.png" % se[i * 4 + j])) / 127.5 - 1.0)
                            image_pol.append(np.expand_dims(np.float32(scipy.misc.imread("./data/0.51m/train/sar_pure/%.8d.png" % se[i * 4 + j])),
                                axis=2) / 127.5 - 1.0)
                    ...
                    for i in range(0, index):
                        ...
                        image_pol = []
                        for j in range(4):
                            image_opt.append(np.float32(scipy.misc.imread("./data/0.51m/train/opt/%.8d.png" % train_index[i])) / 127.5 - 1.0)
                            image_pol.append(np.expand_dims(np.float32(scipy.misc.imread("./data/0.51m/train/sar_pure/%.8d.png" % train_index[i])),
                                axis=2) / 127.5 - 1.0)
                    ...
                    for i in range(0, index):
                        ...
                        image_pol = []
                        for j in range(4):
                            image_opt.append(np.float32(scipy.misc.imread("./data/0.51m/test/opt/%.8d.png" % test_index[i])) / 127.5 - 1.0)
                            image_pol.append(np.expand_dims(np.float32(scipy.misc.imread("./data/0.51m/test/sar_pure/%.8d.png" % test_index[i])),
                                axis=2) / 127.5 - 1.0)
                    ...
                    for i in range(_num_test // 4):
                        ...
                        image_pol = []
                        for j in range(4):
                            image_opt.append(np.float32(scipy.misc.imread("./data/0.51m/test/opt/%.8d.png" % (i * 4 + j + 1))) / 127.5 - 1.0)
                            image_pol.append(np.expand_dims(np.float32(scipy.misc.imread("./data/0.51m/test/sar_pure/%.8d.png" % (i * 4 + j + 1))),
                                axis=2) / 127.5 - 1.0)
